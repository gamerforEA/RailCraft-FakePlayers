...
public abstract class CartTools
{
	...
	public static void setCartOwner(EntityMinecart cart, GameProfile owner)
	{
		if (!cart.worldObj.isRemote)
		{
			...
			{
				CartBase cartBase = (CartBase) cart;
				cartBase.ownerName = owner.getName();
				cartBase.ownerUUID = owner.getId();
			}
			else if (cart instanceof CartContainerBase)
			{
				CartContainerBase cartBase = (CartContainerBase) cart;
				cartBase.ownerName = owner.getName();
				cartBase.ownerUUID = owner.getId();
			}
	...
	public static EntityMinecart placeCart(GameProfile owner, ItemStack cart, WorldServer world, int x, int y, int z)
	{
		...
		{
			boolean placed = cart.getItem().onItemUse(cart, FakePlayerFactory.get(world, railcraftProfile), world, x, y, z, 0, 0, 0, 0);
			if (placed)
			{
	...
	public static void offerOrDropItem(EntityMinecart cart, ItemStack stack)
	{
		...
		if (stack != null && stack.stackSize > 0 && link_A instanceof IItemTransfer) stack = ((IItemTransfer) link_A).offerItem(cart, stack);
		if (stack != null && stack.stackSize > 0 && link_B instanceof IItemTransfer) stack = ((IItemTransfer) link_B).offerItem(cart, stack);
	...
	{
		if (BlockRailBase.func_150049_b_(world, i, j, k)) return isMinecartAt(world, i, j, k, sensitivity, type, subclass);
		return false;
	}
