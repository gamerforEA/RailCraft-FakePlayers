...
public class CartUtils
{
	...
	public static boolean doesCartMatchFilter(ItemStack stack, EntityMinecart cart)
	{
		...
		if (cart instanceof IMinecart) return ((IMinecart) cart).doesCartMatchFilter(stack, cart);
	...
	public static void explodeCart(EntityMinecart cart)
	{
		...
		if (cart instanceof CartBase) ExplosionByPlayer.newExplosion(((CartBase) cart).getOwnerFake(), cart.worldObj, cart, cart.posX, cart.posY, cart.posZ, 3F, true, true);
		else ExplosionByPlayer.newExplosion(null, cart.worldObj, cart, cart.posX, cart.posY, cart.posZ, 3F, true, true);
