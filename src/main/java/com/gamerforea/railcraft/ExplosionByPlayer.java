...
public class ExplosionByPlayer extends Explosion
{
	...
	public void doExplosionA()
	{
		...
		int i0 = MathHelper.floor_double(this.explosionX - (double) this.explosionSize - 1D);
		int i1 = MathHelper.floor_double(this.explosionX + (double) this.explosionSize + 1D);
		int j0 = MathHelper.floor_double(this.explosionY - (double) this.explosionSize - 1D);
		int j1 = MathHelper.floor_double(this.explosionY + (double) this.explosionSize + 1D);
		int k0 = MathHelper.floor_double(this.explosionZ - (double) this.explosionSize - 1D);
		int k1 = MathHelper.floor_double(this.explosionZ + (double) this.explosionSize + 1D);
		...
		for (int i = 0; i < entities.size(); ++i)
		{
			...
			if (distance <= 1D)
			{
				...
				if (d3 != 0D)
				{
					...
					if (!FakePlayerUtils.cantDamage(this.getOwnerPlayer(), entity))
						entity.attackEntityFrom(DamageSource.setExplosionSource(this), (float) ((int) ((d5 * d5 + d5) / 2D * 8D * (double) this.explosionSize + 1D)));
		...
		for (int i = 0; i < 16; ++i)
		{
			for (int j = 0; j < 16; ++j)
			{
				for (int k = 0; k < 16; ++k)
				{
					...
					{
						double d0 = (double) ((float) i / 30F - 1F);
						double d1 = (double) ((float) j / 30F - 1F);
						double d2 = (double) ((float) k / 30F - 1F);
						...
						for (float f = 0.3F; size > 0F; size -= f * 0.75F)
						{
							...
							{
								if (!FakePlayerUtils.cantBreak(x, y, z, this.getOwnerPlayer()))
	...
	public EntityPlayer getOwnerPlayer()
	{
		...
		if (this.ownerPlayer != null)
			...
			player = FakePlayerUtils.getModFake(this.worldObj);
		return player;
	}
